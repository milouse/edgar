...
Finally, ~edgar~ understands sub-hosts and ansible-like ~loop~ (or legacy
~with_items~) processing:
...
- ~loop~ :: (or legacy ~with_items~) define that the current block configuration
  must be duplicated for each item of this list. Value must be something python
  is able to iterate over (a list, a range expression…). You can use the
  ~{item}~ tag in any option value of the same block.
